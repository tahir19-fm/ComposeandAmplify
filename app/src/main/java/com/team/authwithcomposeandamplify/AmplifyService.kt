...
interface AmplifyService {
    ...
    fun fetchUserState(onComplete: (Boolean) -> Unit)
}
...
class AmplifyServiceImpl : AmplifyService {
    ...
    override fun fetchUserState(onComplete: (Boolean) -> Unit) {
        Amplify.Auth.fetchAuthSession({onComplete(it.isSignedIn)},{ Log.d("arhar", "fetchUserState:$it ")})
    }
